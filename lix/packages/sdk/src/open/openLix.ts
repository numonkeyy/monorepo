...
export async function openLix(args: {
	...
	let hasMoreEntries: number | undefined = undefined;
	let pending: Promise<void> | undefined;
	let resolve: () => void;
	let runNumber = 1;
	async function queueWorker(trail = false) {
		// console.log({ pending, hasMoreEntries, runNumber });

		if (pending && !trail) {
			hasMoreEntries = runNumber;
			// console.log({ hasMoreEntries });
			return;
		}
		runNumber++;
		...
		if (!hasMoreEntries || (numEntries === 0 && hasMoreEntries < runNumber)) {
			resolve!(); // TODO: fix type
			pending = undefined;
			hasMoreEntries = undefined;
			// console.log("resolving");
		}

		// TODO: handle endless tries on failing quee entries
		setTimeout(() => queueWorker(true), hasMoreEntries ? 0 : 1000);
	}
	queueWorker();
