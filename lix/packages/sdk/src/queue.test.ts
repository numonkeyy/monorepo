...
test("should use queue and settled correctly", async () => {
	...
	const queue = await lix.db.selectFrom("queue").selectAll().execute();
	...
	expect((await lix.db.selectFrom("queue").selectAll().execute()).length).toBe(
		0,
	);
	...
	const queue2 = await lix.db.selectFrom("queue").selectAll().execute();
