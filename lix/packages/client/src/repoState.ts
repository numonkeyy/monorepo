...
export async function repoState(
	...
	if (freshClone) {
		...
		// FIXME: symlink = false in git/config!??? https://linear.app/opral/issue/LIX-70/symlink-=-false-in-gitconfig
		await isoGit
			.clone({
				fs: rawFs,
				http: makeHttpClient({
	...
	function delayedAction({
		...
		// this allows building isogit apis directly requesting obejcts that are missing and lazy loaded, currently not used as we handle this allready in the replacement step of the placeholders
		// else if (
		// 	experimentalFeatures.lazyClone &&
		// 	typeof rootObject !== "undefined" &&
