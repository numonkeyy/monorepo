...
export async function repoState(
	...
	// todo: discussion: use functions or repo state for this?!
	// state currentRef
	...
	async function ensureFirstBatch() {
		...
		console.log(gitignores, state.pending, nextBatch, state.checkedOut, freshClone)
		if (state.pending) {
	...
	async function doCheckout() {
		...
		for (const placeholder of allBatchFiles.filter((entry) => rawFs._isPlaceholder?.(entry))) {
			...
			await rawFs.rm(placeholder).catch((err) => {})
		}
		...
		if (allBatchFiles.length > 0) {
			res = await checkout({
				...
				console.log({ error, allBatchFiles })
			})
		}
