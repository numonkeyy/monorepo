...
import { checkout, listFiles } from "../../vendored/isomorphic-git/index.js"
...
import { basename, dirname, blobExistsLocaly } from "./helpers.js"
...
export const withLazyFetching = (
	...
	async function lazyLoadingInterceptor({
		...
		const base = basename(gitFilePath)
		const filePathDir = dirname(gitFilePath)
		const folders = filePathDir.split("/")
		...
		if (!gitFilePath.includes(".git")) {
			...
			// jan: todo: only reqyired to ad hoc copy the walk function here.
