...
export async function _statusList({
	...
	try {
		// this call will materialzie all gitignore files that are queued on lazy cloning, if this is required at more places we should refactor repo to expose explicit wait for pending materializations call
		await nodeishFs.readFile(".gitignore").catch((err) => {
			// ignore not existing gitignores
		})
