...
export function openRepo (url, { branch, author }) {
  ...
  async function updateStatus(addStatus) {
    ...
    if (addStatus) {
      ...
      const originCommits = (await state.repo.log({ ref: 'origin/' + state.currentBranch, depth }))
      const originIndex = originCommits.reduce((agg, com) => {
        agg[com.oid] = com
        return agg
      }, {})

      const headCommits = await state.repo.log({ depth })
      const headIndex = headCommits.reduce((agg, com) => {
      ...
      const localHead = headCommits[0].oid
