<script lang="ts">
  ...
  $effect(() => {
    // (selectedRef && repo?.currentBranch && (selectedRef !== repo?.currentBranch))
    ...
    if (repos[selectdRepo] && (opened !== repos[selectdRepo]?.full_name)) {
      ...
      refetcher = setTimeout(() => { // setInterval
        metaLoaded = true
        //   repo.fetch()
      }, 1500)
      ...
      <aside id="lix-sidebar">
        ...
        <div style="margin-bottom: 26px; margin-left: -54px;">
          {#if repo?.currentBranch}
            <select style="max-width: 100px; padding: 2px;" bind:value={selectedRef} on:focus={repo.listBranches} on:mouseover={repo.listBranches}>
              {#each repo.branches as { title, branchName }}
                <option value={branchName}>{title}</option>
              {/each}
            </select>
          {/if}
