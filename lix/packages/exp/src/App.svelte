    ...
    <aside id="lix-sidebar">
      ...
      <div style="width: 100%">
        {#each repo.commits as { commit, origin, oid, current }, i}
          <div style="font-size: 11px; margin-bottom: 33px; height: 32px;" class="commit" title="Id:{oid}   Hash:{commit.tree}   Parents:{JSON.stringify(commit.parent)}">
            ...
            {#if commit.message.length > 0}
              <p style="margin-bottom: 12px;">{commit.message}</p>
            {/if}
          </div>
        {/each}
      </div>
